...
import { ISkillData } from '../../pages/auth/skills';
...
type IModalProps = {
  skill: ISkill;
  closeModal(): void;
  newSkill(data: ISkillData, file: File): void;
  editSkill(data: ISkillData, file: File): void;
};
...
export function SkillsModal({ skill, closeModal, editSkill, newSkill }: IModalProps) {
  ...
  function handleEditSkill(data: ISkillData) {
    ...
    editSkill({ ...data, id: skill.id }, file);
  }

  function handleNewSkill(data: ISkillData) {
    const { name, description, url } = data;
  ...
  return (
    <>
      <Container onSubmit={skill.id ? handleSubmit(handleEditSkill) : handleSubmit(handleNewSkill)}>
